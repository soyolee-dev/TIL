...
단, 현재 디렉토리 중 변경된 파일들만 선택되기 때문에 사용 전 git status를 확인해본다
...
: Staging Area에 add한 파일을 Working Directory로 복원
...
`$ git add .` 의 경우 현재 디렉토리 중 변경된 파일들만 적용되기 때문에 status를 꼭 확인하자
